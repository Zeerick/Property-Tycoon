...
public class TurnOrderScript : MonoBehaviour
{
    ...
    void Update()
    {
        gameObject.GetComponent<UnityEngine.UI.Text>().text = player.GetComponent<PlayerScript>().name + " £" + player.GetComponent<PlayerScript>().money.ToString();
    ...
    {
        player.GetComponent<PlayerScript>().name = gameObject.transform.Find("Change Name").gameObject.transform.Find("InputField").gameObject.GetComponent<InputField>().text;
    }
}
